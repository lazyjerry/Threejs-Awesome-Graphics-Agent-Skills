...
const atmosphereMaterial = new THREE.ShaderMaterial({
  ...
  fragmentShader: `
    ...
    float rayleighPhase(float cosine) {
      return 0.05968310366 * (1.0 + cosine * cosine);
    }
    float miePhase(float cosine, float g) {
      float g2 = g * g;
      return 0.07957747155 * (1.0 - g2) /
        pow(1.0 + g2 - 2.0 * g * cosine, 1.5);
    }

    void main() {
      vec3 rayOrigin = cameraPosition;
      vec3 rayDirection = normalize(vWorldPosition - cameraPosition);
      vec2 atmosphereHit = raySphere(
        rayOrigin,
        rayDirection,
        uAtmosphereRadius
      );
      vec2 groundHit = raySphere(
        rayOrigin,
        rayDirection,
        uPlanetRadius
      );
      float start = max(atmosphereHit.x, 0.0);
      float end = atmosphereHit.y;
      if (groundHit.x > 0.0) end = min(end, groundHit.x);
      float rayLength = max(end - start, 0.0);
      if (rayLength <= 0.0) discard;

      // Convert per-kilometre coefficients into this unit-radius world.
      vec3 betaR = vec3(0.0058, 0.0135, 0.0331) * 12.0;
      vec3 betaM = vec3(0.0022) * 8.0;
      vec3 betaMExtinction = vec3(0.0032) * 8.0;
      float scaleHeightR = 0.028;
      float scaleHeightM = 0.011;
      float viewStep = rayLength / 16.0;
      float opticalR = 0.0;
      float opticalM = 0.0;
      vec3 inScatter = vec3(0.0);
      float mu = dot(rayDirection, normalize(uSunDirection));
      float phaseR = rayleighPhase(mu);
      float phaseM = miePhase(mu, 0.76);

      for (int index = 0; index < 16; index += 1) {
        float distanceAlongRay =
          start + (float(index) + 0.5) * viewStep;
        vec3 samplePosition =
          rayOrigin + rayDirection * distanceAlongRay;
        float height = max(
          length(samplePosition) - uPlanetRadius,
          0.0
        );
        float densityR = exp(-height / scaleHeightR);
        float densityM = exp(-height / scaleHeightM);
        opticalR += densityR * viewStep;
        opticalM += densityM * viewStep;

        vec2 sunHit = raySphere(
          samplePosition,
          normalize(uSunDirection),
          uAtmosphereRadius
        );
        float sunLength = max(sunHit.y, 0.0);
        float sunStep = sunLength / 4.0;
        float sunOpticalR = 0.0;
        float sunOpticalM = 0.0;
        for (int lightIndex = 0; lightIndex < 4; lightIndex += 1) {
          vec3 sunPosition = samplePosition +
            normalize(uSunDirection) *
            ((float(lightIndex) + 0.5) * sunStep);
          float sunHeight = max(
            length(sunPosition) - uPlanetRadius,
            0.0
          );
          sunOpticalR += exp(-sunHeight / scaleHeightR) * sunStep;
          sunOpticalM += exp(-sunHeight / scaleHeightM) * sunStep;
        }
        vec2 planetShadow = raySphere(
          samplePosition,
          normalize(uSunDirection),
          uPlanetRadius
        );
        float sunVisible = planetShadow.x > 0.0 ? 0.0 : 1.0;
        vec3 extinction =
          betaR * (opticalR + sunOpticalR) +
          betaMExtinction * (opticalM + sunOpticalM);
        vec3 transmittance = exp(-extinction) * sunVisible;
        inScatter +=
          (betaR * densityR * phaseR +
            betaM * densityM * phaseM) *
          transmittance * viewStep;
      }

      vec3 color = inScatter * 22.0;
      color = color / (vec3(1.0) + color * 0.22);
      float alpha = clamp(
        max(max(color.r, color.g), color.b) * 1.15,
        0.0,
        0.96
      );
      if (uDebugMode != 6 && uDebugMode != 0) discard;
...
const atmosphere = new THREE.Mesh(
  new THREE.SphereGeometry(atmosphereRadius, 160, 80),
  atmosphereMaterial,
);
...
function setDebugMode(modeName) {
  ...
  atmosphereUniforms.uDebugMode.value = mode;
}
  ...
  return {
      ...
      metrics() {
        return { tier: "terrestrial far LOD / 16x4 atmosphere" };
      },
  };
}
