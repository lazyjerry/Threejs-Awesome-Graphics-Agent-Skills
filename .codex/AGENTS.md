...
- every example when viewed inside the dev shim gallery should have a standard scene setup with pannanble and orbitable camera, camera shouldn't be allowed to move beyond scene (like under the ground surface), full canvas inside the scene viewport. decorative assets and implementations in dev shim can be added if it's deemed important for real-time view experience, otherwise are not strictly necessary
