...
- any examples included in skills need only the effect implementation itself, WITHOUT runtime, supporting static assets, or supporting scene implementation. Runtime, supporting static assets, and supporting scene implementation are owned by dev shim. They are for dev visual inspection only, not a part of the skill pack
